...
import com.ebiz.bp_mysql.web.util.FtpImageUtils;
...
public abstract class BaseAction extends BaseSsiAction {
	...
	protected void setNaviStringToRequestScope(HttpServletRequest request, String oper_name) {
		...
		if (StringUtils.isNotBlank(mod_id)) {
			StringBuffer naviString = new StringBuffer("");
			if (StringUtils.isNotBlank(mod_id)) {
				SysModule sysModule = new SysModule();
				sysModule.setMod_id(new Integer(mod_id));
				List<SysModule> sysModuleList = getFacade().getSysModuleService().proGetSysModelParentList(sysModule);
				naviString.append(StringHelper.getNaviStringFromSysModuleList(sysModuleList));
			}
			request.setAttribute("naviString", naviString.append(oper_name).toString());
		}
	}
