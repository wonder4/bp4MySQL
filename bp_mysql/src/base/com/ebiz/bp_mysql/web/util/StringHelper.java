...
public class StringHelper {
	...
	public static String getjQzTreeNodesFromSysModuleList(List<SysModule> sysModuleList) {
		...
		for (SysModule sysModule : sysModuleList) {
			...
			if (!"0".equals(par_id)) {
				// { mod_id:1, par_id:0, name:"基本功能演示", "url":"",
				// "target":"mainFrame"},
				sb.append("{");
	...
	/**
	 * @param facade
	 * @param cls_scope 1:产品
	 * @param pd_class_type par_id
	 * @param isCanSelectedAll true都可以选择，false如果是锁定的选项不让选择
	 * @return
	 */
	public static String getTreeNodesFromBasePdClassList(Facade facade, Integer cls_scope, String pd_class_type,
		...
		basePdClass.setPar_id(Integer.valueOf(pd_class_type));
		List<BaseClass> basePdClassList = facade.getBaseClassService().proGetBaseClassSonList(basePdClass);
		for (BaseClass entity : basePdClassList) {
			...
			if (!pd_class_type.equals(par_id)) {
				// { cls_id:1, par_id:0, name:"基本功能演示", "url":"",
				// "target":"mainFrame"},
				sb.append("{");
	...
	public static String getTreeNodesFromDeptInfoList(List<DeptInfo> helpModuleList) {

		StringBuffer sb = new StringBuffer();

		for (DeptInfo helpModule : helpModuleList) {

			String _id = String.valueOf(helpModule.getDept_id());
			String _par_id = String.valueOf(helpModule.getPar_id());

			if ("0".equals(_par_id)) {
				_par_id = "-1";
			}
			String _text = StringUtils.replace(helpModule.getDept_name(), ":", "&#58;");
			if (StringUtils.isEmpty(_text)) {
				continue;
			}
			String _hint = _text;
