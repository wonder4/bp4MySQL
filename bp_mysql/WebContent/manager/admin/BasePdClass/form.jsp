      ...
      <tr>
        ...
        <tr>
          ...
          <td colspan="2" width="88%"><html-el:hidden property="par_id" styleId="par_id"/>
            <c:if test="${not empty af.map.cls_id}"><html-el:text property="par_name" styleId="par_name" maxlength="125" style="width:280px" styleClass="webinput" readonly="true"/></c:if>
            <c:if test="${empty af.map.cls_id}"><html-el:text property="par_name" styleId="par_name" maxlength="125" style="width:280px" styleClass="webinput" onclick="getParClsInfo()" readonly="true"/></c:if>
          </td>
        </tr>
      <tr>
...
function getParClsInfo() {
	var url = "CsAjax.do?method=getParClsInfo&isSelectAll=true&pd_class_type="  + '${af.map.pd_class_type}' + "&isSelectAll=true&azaz=" + Math.random() + "&time=" + new Date().getTime();
	asyncbox.open({
