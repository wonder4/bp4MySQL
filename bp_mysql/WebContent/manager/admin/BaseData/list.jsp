    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClassSearch">
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" align="left">
            <tr>
              <td width="6%" nowrap="nowrap"><c:if test="${af.map.type eq 10}">用户类型：</c:if>
  ...
  <form id="listForm" name="listForm" method="post" action="BaseData.do?method=delete">
    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClass">
      <tr>
        ...
        <th width="8%" nowrap="nowrap"><c:if test="${af.map.type eq 10}">用户代码</c:if></th>
        <th nowrap="nowrap"><c:if test="${af.map.type eq 10}">用户类型</c:if></th>
        <th width="11%">备注</th>
        <th width="10%">添加时间</th>
        <th width="6%">排序值</th>
        <th width="6%">是否锁定</th>
      ...
      <c:forEach var="cur" items="${baseDataList}" varStatus="vs">
        <tr>
          ...
          <td align="center"><c:out value="${cur.type_value}"></c:out></td>
