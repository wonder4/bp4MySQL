  ...
  <html-el:form action="/admin/PdInfo" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="tableClassTbody">
      ...
      <tr>
        ...
        <td colspan="2" width="88%"><html-el:text property="cls_name" styleId="cls_name" readonly="true" onclick="getParClsInfo();" style="cursor:pointer;" styleClass="webinput"/></td>
      </tr>
      ...
      <!--<tr>
        <td width="12%" nowrap="nowrap" class="title_item"><span style="color: #F00;">*</span>所属企业：</td>
        <td colspan="2" width="88%"><html-el:select property="own_entp_id" styleId="own_entp_id" value="${af.map.own_entp_id}">
            <html-el:option value="">请选择...</html-el:option>
            <c:forEach items="${entpInfoList}" var="cur">
              <html-el:option value="${cur.id}">
                <c:out value="${cur.entp_name}"/>
              </html-el:option>
            </c:forEach>
          </html-el:select></td>
      </tr>
...
function getParClsInfo() { 
	var url = "CsAjax.do?method=getParClsInfo&hideParentNode=true&pd_class_type="  + '${af.map.pd_class_type}' + "&isSelectAll=true&azaz=" + Math.random() + "&time=" + new Date().getTime();
	asyncbox.open({
